...
impl Item for GroupBoxItem {
    ...
    fn layout_info(
        ...
        let padding = unsafe { Pin::new_unchecked(&self.padding).get() }.get() * 2.0; // 左右或上下
...
// }
//
impl crate::View for GroupBox {
    type ItemType = GroupBoxItem;

    fn create_item(self) -> Self::ItemType {
        let item = GroupBoxItem::default();

        // 应用配置属性
        item.title.set(self.title);

        item
    }

    fn into_item_tree(self) -> Option<crate::ItemTreeRc> {
        // GroupBox 使用 SingleItemTree
...
mod tests {
    ...
    fn test_group_box_view() {
        let group_box = GroupBox::new().set_title("My Group");
        assert_eq!(group_box.title.as_str(), "My Group");
        // View trait implementation allows creating ItemTree
        let _tree = group_box.into_item_tree();
    }
}
