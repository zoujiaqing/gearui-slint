// GearUI HorizontalLayout - 水平布局容器
//
// 对应 Slint 的 HorizontalLayout 控件，提供子控件的水平排列功能
//
// 字段映射表：
...
/// Layer 1: Core HorizontalLayout Item Implementation
///
/// 实现 Item trait，提供实际的水平布局逻辑。
/// 字段名和默认值完全匹配 Slint 的 HorizontalLayout 规范。
#[repr(C)]
#[derive(FieldOffsets, Default, SlintElement)]
#[pin]
pub struct HorizontalLayoutItem {
...
impl Item for HorizontalLayoutItem {
...
impl ItemConsts for HorizontalLayoutItem {
    const cached_rendering_data_offset: const_field_offset::FieldOffset<Self, CachedRenderingData> =
        Self::FIELD_OFFSETS.cached_rendering_data.as_unpinned_projection();
}

declare_item_vtable! {
    fn slint_get_GearUIHorizontalLayoutVTable() -> GearUIHorizontalLayoutVTable for HorizontalLayoutItem
}

/// Layer 2: HorizontalLayout View Control Wrapper
///
/// 用户友好的 HorizontalLayout 控件 API，提供构建器模式。
#[derive(Clone)]
...
impl HorizontalLayout {
    /// Create a new HorizontalLayout instance with Slint-compliant default values
    pub fn new() -> Self {
        Self {
    ...
    /// 🎯 Phase 8: 添加子控件到水平布局中
    ///
    /// 这是核心的组合功能，允许用户添加任意控件到水平布局中。
    ///
    /// 使用示例：
    /// ```rust
    /// let layout = HorizontalLayout::new()
    ///     .with_child(Text::new().with_text("左侧"))
    ///     .with_child(Button::new().with_text("按钮"))
    ///     .with_child(Text::new().with_text("右侧"));
    /// ```
    ...
    /// Create a HorizontalLayoutItem with this HorizontalLayout's properties
    fn create_item_internal(&self) -> HorizontalLayoutItem {
        let item = HorizontalLayoutItem::default();
...
impl crate::View for HorizontalLayout {
    type ItemType = HorizontalLayoutItem;
    ...
    fn build(self) -> Self {
        ...
        println!("🎯 HorizontalLayout::build() - 递归构建 {} 个子控件", children_count);
    ...
    fn into_item_tree(self) -> Option<i_slint_core::item_tree::ItemTreeRc> {
        println!(
            "🎯 HorizontalLayout::into_item_tree: Creating MultiItemTree with {} children",
            self.children.len()
        );
        ...
        if self.children.is_empty() {
            ...
            println!("🎯 HorizontalLayout has no children, using CompositeItemTree");
            ...
            println!(
                "🎯 HorizontalLayout has {} children, using MultiItemTree",
                self.children.len()
            );

            // 创建 HorizontalLayout 控件
            let layout_item = self.create_item_internal();
            ...
            println!("✅ HorizontalLayout::into_item_tree: MultiItemTree created successfully");
            Some(multi_tree)
        }
    }
}

/// Helper functions for quick HorizontalLayout creation
pub mod helpers {
    use super::*;

    /// Create a horizontal layout with spacing
    pub fn horizontal_layout_with_spacing(spacing: f32) -> HorizontalLayout {
        HorizontalLayout::new().with_spacing(spacing)
    }

    /// Create a center-aligned horizontal layout
    pub fn center_horizontal_layout() -> HorizontalLayout {
        HorizontalLayout::new().with_alignment(LayoutAlignment::Center)
    }
}
